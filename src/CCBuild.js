...
CCBuild.prototype._compile = function (compilerConfiguration) {
    ...
    compiler.run(function (code, stdout, stderr) {
        ...
        Q.all(currentWarningsFilterFiles.map(function (filePath) {
            ...
            .then(function () {
                var gccResultProcessor = new GCCResultProcessor(code, stdout, stderr);
                ...
                if (code !== 0) {
                    var err = new Error(code + (filteredStderr ? ': ' + filteredStderr : ''));
                    /**
                     ...
                     */
                    self.emit('compiled', compilerConfiguration.unitName, stdout, filteredStderr);
